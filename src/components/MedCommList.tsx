    ...
    <div className="space-y-3">
      {calls.map(call => (
        <div 
          key={call.id} 
          className={cn(
            "bg-white rounded-xl shadow-sm border p-4 transition-all hover:shadow-md",
            call.urgencyFlag ? "border-red-300 ring-1 ring-red-50 ring-inset" : "border-gray-100"
          )}
        >
          <div className="flex flex-col md:flex-row gap-4 justify-between">
            {/* Left: Basic Info */}
            <div className="flex items-start gap-4">
              <div className="flex flex-col items-center justify-center min-w-[60px] bg-gray-50 rounded-lg p-2 border border-gray-100">
                <span className="text-xl font-bold text-gray-900">{call.initials}</span>
                <span className="text-xs font-medium text-gray-500">{call.age}y {call.sex}</span>
              </div>
              
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className={cn("px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border", getStatusColor(call.status))}>
                    {call.status}
                  </span>
                  {call.urgencyFlag && (
                    <span className="bg-red-600 text-white px-2 py-0.5 rounded text-[10px] font-bold uppercase flex items-center gap-1">
                      <AlertTriangle size={10} /> URGENT
                    </span>
                  )}
                  {call.traumaActivation && call.traumaActivation !== 'none' && (
                    <span className="bg-orange-600 text-white px-2 py-0.5 rounded text-[10px] font-bold uppercase flex items-center gap-1">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-600">
                  ...
                  <span className="flex items-center gap-1 font-bold text-blue-600"><Clock size={12} /> ETA: {call.eta}</span>
                  <span className="flex items-center gap-1"><User size={12} className="text-gray-400" /> {call.takenBy}</span>
                </div>
              ...
              <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-2">
                {Object.entries(call.vitals).map(([key, value]) => (
                  <div key={key} className="flex flex-col items-center">
                    <span className="text-[8px] font-bold text-gray-400 uppercase">{key}</span>
            ...
            <div className="flex flex-row md:flex-col gap-2 justify-end">
              ...
              {call.convertedToPatientId && (
                <div className="flex items-center justify-center gap-2 px-4 py-2 bg-green-50 text-green-700 rounded-lg text-xs font-bold border border-green-200">
