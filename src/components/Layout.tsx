...
export const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, user, onLogout, onAddTeamMember, activeShiftId }) => {
  ...
  return (
    <div className="min-h-[100dvh] bg-gray-50 dark:bg-gray-950 flex flex-col transition-colors">
        ...
        <div className="max-w-7xl mx-auto px-4 h-10 md:h-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="flex items-center gap-2 px-2 py-0.5 md:px-3 md:py-1 bg-gray-50 rounded-full border border-gray-100">
              ...
              <span className="text-[10px] md:text-xs font-bold text-gray-700 hidden sm:block">{user?.email?.split('@')[0]}</span>
            </div>
          </div>
        </div>
      </header>
      ...
      {/* Main Content Area */}
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 pt-0 pb-24 md:pb-4">
        {children}
      </main>

      {/* Bottom Navigation (Mobile Friendly) */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-1 z-30 md:relative md:border-t-0 md:bg-transparent md:max-w-7xl md:mx-auto md:w-full md:px-4 md:py-4">
        <div className="flex items-center justify-around md:justify-start md:gap-4 max-w-lg mx-auto md:mx-0">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={cn(
                "flex flex-col md:flex-row items-center gap-0.5 md:gap-2 px-4 py-1.5 rounded-xl transition-all",
                activeTab === tab.id 
                  ? "text-blue-600 bg-blue-50 md:bg-blue-600 md:text-white md:shadow-md" 
                  : "text-gray-400 hover:text-gray-600 hover:bg-gray-50"
              )}
            >
              <div className={cn("transition-all duration-300", !isAtBottom && "md:block hidden")}>
                {tab.icon}
              </div>
