...
export const MedCommForm: React.FC<MedCommFormProps> = ({ onSubmit, onCancel, initialData }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden max-w-2xl mx-auto">
      <div className="bg-blue-600 p-4 text-white flex items-center justify-between">
        <h2 className="text-lg font-bold flex items-center gap-2">
          <Phone size={20} /> {initialData ? 'Edit MedComm Call' : 'New MedComm Call'}
        </h2>
        <div className="flex items-center gap-2">
          <label className="flex items-center gap-1.5 cursor-pointer bg-blue-700 px-3 py-1.5 rounded-full text-xs font-bold hover:bg-blue-800 transition-colors">
            <input 
              type="checkbox" 
              checked={formData.urgencyFlag}
              onChange={(e) => handleChange('urgencyFlag', e.target.checked)}
              className="w-4 h-4 rounded border-white text-red-500 focus:ring-red-500"
            />
      ...
      <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase">Initials</label>
            <input 
              required
              className="w-full p-3 border border-gray-200 rounded-lg text-lg font-bold focus:ring-2 focus:ring-blue-500 outline-none"
              placeholder="AB"
        ...
        <div className="space-y-2">
          ...
          {formData.referredFrom === 'Other' && (
            <input 
              className="w-full mt-2 p-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none animate-in slide-in-from-top-2"
        ...
        <div className="space-y-1">
          ...
          <div className="flex gap-2">
            ...
            />
            <input 
              className="w-32 p-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none"
        ...
        <div className="space-y-1">
          ...
          <p className="text-[10px] text-red-500 font-bold mb-1 uppercase">⚠️ No PHI beyond initials/age</p>
          <textarea 
            className="w-full p-3 border border-gray-200 rounded-lg text-sm h-24 focus:ring-2 focus:ring-blue-500 outline-none"
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
            {[
              ...
              <div key={v.field} className="space-y-1">
                <span className="text-[10px] font-bold text-gray-400 block text-center">{v.label}</span>
                <input 
                  className="w-full p-2 border border-gray-200 rounded-md text-xs text-center focus:ring-1 focus:ring-blue-500 outline-none"
              ...
              <button
                ...
                className={cn(
                  ...
                  formData.traumaActivation === level 
                    ? level === 'none' ? "bg-gray-100 border-gray-400 text-gray-700" : "bg-red-50 border-red-500 text-red-700 shadow-sm"
        ...
        <div className="space-y-1">
          <label className="text-xs font-bold text-gray-500 uppercase">Interventions / Labs / Imaging</label>
          <textarea 
            className="w-full p-3 border border-gray-200 rounded-lg text-sm h-20 focus:ring-2 focus:ring-blue-500 outline-none"
      ...
      <div className="p-4 bg-gray-50 border-t border-gray-200 flex gap-3">
        <button 
          type="button"
          onClick={onCancel}
          className="flex-1 p-3 rounded-xl border border-gray-300 text-gray-700 font-bold hover:bg-gray-100 transition-colors"
        >
          Cancel
        </button>
        <button 
          type="submit"
          className="flex-[2] p-3 rounded-xl bg-blue-600 text-white font-bold flex items-center justify-center gap-2 hover:bg-blue-700 transition-colors shadow-md"
        >
